    ...
    if request.method == 'POST':
        ...
        try:
            ...
            transaction = Transaction(
                order_id=order, amount=amount, transaction_status='success')
            transaction.save()
            order.status = 'paid'
            order.save()
...
def get_range_revenue(request):
    if request.method == 'GET':
        ...
        transactions = Transaction.objects.filter(
            status='paid', created_at__range=[start_date, end_date])
        range_revenue = 0
        for transaction in transactions:
...
def get_device_revenue(request):
    if request.method == 'GET':
        ...
        transactions = Transaction.objects.filter(
            status='paid', device_id=device_id)
        device_revenue = 0
        for transaction in transactions:
    ...
    def get(self, request):
        ...
        if device_id:
            revenues = Transaction.objects.filter(
                device_id=device_id, status='paid')
        else:
