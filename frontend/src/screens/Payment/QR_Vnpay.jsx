...
function QR_Vnpay() {
    ...
    useEffect(() => {
        const fetchQRPayment = async () => {
            try {
                ...
                const response = await fetch(`${import.meta.env.VITE_REACT_APP_BACKEND}/vnpay/api?device=${deviceNumber}&amount=${framePrice}`);
    ...
    useEffect(() => {
        const checkPaymentStatus = async (orderCodeNum) => {
            try {
                const response = await fetch(`${import.meta.env.VITE_REACT_APP_BACKEND}/vnpay/api/webhook?order=${orderCodeNum}`);
